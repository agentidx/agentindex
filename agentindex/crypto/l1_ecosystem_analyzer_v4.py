#!/usr/bin/env python3
"""
NERQ L1 ECOSYSTEM ANALYZER v4
==============================
v4 CHANGES from v3:
  - FIX: L1 native tokens with platforms={"":""} now auto-mapped to own chain
  - FIX: 17 new chains added (bitcoin, xrp, doge, ltc, bch, xlm, hbar, etc.)
  - FIX: Cross-chain L1 tokens filtered (BNB on Ethereum no longer inflates ETH ecosystem)
  - FIX: ETH/SOL crash scenarios now cascade to L2s
  - KEEP: All v3 features (moat, cycle context, risk/reward, investment signals)
  - NEW: Coverage check built-in (--coverage flag)
...
Usage:
    python3 l1_ecosystem_analyzer_v4.py --all --save     # Full analysis + save
    python3 l1_ecosystem_analyzer_v4.py --insights       # Insights 1-8
    python3 l1_ecosystem_analyzer_v4.py --moat            # Insights 9-12 (moat/cycle/R-R)
    python3 l1_ecosystem_analyzer_v4.py --coverage        # Top-100 coverage check
    python3 l1_ecosystem_analyzer_v4.py --save            # Save to DB only
"""
...
# ═══════════════════════════════════════════════════════════════
# L1/L2 CHAIN DEFINITIONS — v4: expanded from 21 → 38 chains
# ═══════════════════════════════════════════════════════════════
...
L1_CHAINS = {
    ...
    # === v4 NEW CHAINS (17) — fixes top-100 coverage gap ===
...
def main():
    ...
    if run_coverage:
        ...
        total_top100_mcap = covered_mcap + missing_mcap

        print(f"  Covered: {len(covered)}/100 tokens ({covered_mcap/total_top100_mcap*100:.1f}% of mcap = {fmt_b(covered_mcap)})")
        print(f"  Missing: {len(missing)}/100 tokens ({missing_mcap/total_top100_mcap*100:.1f}% of mcap = {fmt_b(missing_mcap)})")

        if missing:
            print(f"\n  MISSING TOKENS:")
            for t in sorted(missing, key=lambda x: x.get("market_cap_usd", 0) or 0, reverse=True):
                m = t.get("market_cap_usd", 0) or 0
                plat = ""
                try:
    ...
    if run_scenarios:
        ...
        for target_chain in ["ethereum", "solana", "binance-smart-chain"]:
