...
`/dependencies/{slug}.json` from `endpoints.dependencies`). Tests and
future modular deploys import `app` from here; the production tree
still serves everything through `agentindex.api.discovery` for now,
...
from agentindex.api.endpoints.dependencies import router as dependencies_router
