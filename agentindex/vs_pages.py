...
def mount_vs_pages(app):
    ...
    # ================================================================
    # SAFETY COMPARISON ALIAS: /is-{a}-or-{b}-safer
    # ================================================================
    @app.get("/is-{slug_a}-or-{slug_b}-safer", response_class=HTMLResponse)
    def is_a_or_b_safer(slug_a: str, slug_b: str):
        """SEO alias: /is-langchain-or-autogpt-safer → VS comparison page."""
        session = get_session()
        try:
            # Resolve slugs to agent IDs by name matching
            agent_a = _fetch_agent_by_slug(session, slug_a)
            agent_b = _fetch_agent_by_slug(session, slug_b)
            if not agent_a or not agent_b:
                return HTMLResponse(status_code=404,
                    content="<h1>Comparison not found</h1><p><a href='/vs'>Browse comparisons</a></p>")
            j_a = _fetch_jurisdictions(session, str(agent_a["id"]))
            j_b = _fetch_jurisdictions(session, str(agent_b["id"]))
            html = _render_vs_page(agent_a, agent_b, j_a, j_b)
            return HTMLResponse(content=html)
        finally:
            session.close()

    logger.info("VS pages mounted: /vs, /vs/{id_a}/{id_b}, /is-{a}-or-{b}-safer, /sitemap-vs.xml")
...
def _fetch_agent_by_slug(session, slug):
    ...
    row = session.execute(text("""
        ...
        AND (LOWER(name) = :exact OR LOWER(name) LIKE :pattern)
        ORDER BY CASE WHEN LOWER(name) = :exact THEN 0 ELSE 1 END,
                 trust_score_v2 DESC NULLS LAST
        LIMIT 1
    """), {"exact": clean.lower(), "pattern": f"%{clean.lower()}%"}).fetchone()
    if not row:
        return None
...
def _render_vs_index(pairs):
    ...
    for a, b in pairs[:100]:
        rows += f"""<tr>
<td><a href="/vs/{a['id']}/{b['id']}" style="color:#0d9488;text-decoration:none;font-weight:500">{_esc(_short_name(a['name']))} vs {_esc(_short_name(b['name']))}</a></td>
...
def _render_vs_page(a, b, j_a, j_b):
    ...
    for j_id in all_j_ids:
        ...
        if sa < sb:
            a_wins += 1
            highlight = f' style="background:#ecfdf5"'
        elif sb < sa:
            b_wins += 1
...
<a href="/" style="font-weight:700;color:#0d9488;text-decoration:none">nerq</a> &rsaquo; <a href="/vs">Comparisons</a> &rsaquo; <strong>{_esc(name_a)} vs {_esc(name_b)}</strong>
</div>
...
<a href="/agent/{a['id']}" style="display:block;padding:16px;background:#f9fafb;border:1px solid #e5e7eb;text-decoration:none;color:#1a1a1a">
...
<a href="/agent/{b['id']}" style="display:block;padding:16px;background:#f9fafb;border:1px solid #e5e7eb;text-decoration:none;color:#1a1a1a">
