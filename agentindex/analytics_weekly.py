...
def _get_cached_data():
    ...
    Cache is rebuilt by com.nerq.analytics-weekly-cache LaunchAgent every
    ~25 min. If cache is stale or missing, returns cached data anyway (or
    empty placeholder) rather than blocking the HTTP request on a 100+
    second SQLite query. Background: 524 incident 2026-04-09, fallback
    caused death spiral when cache rebuild itself was timing out.
    """
    try:
        ...
        logger.warning(f"Analytics weekly cache read failed: {e}")
