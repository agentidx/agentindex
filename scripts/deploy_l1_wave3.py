#!/usr/bin/env python3
"""
deploy_l1_wave3.py — L1 Wave 3 registry unlock runner (full non-skip sweep).

Widens the com.nerq.api LaunchAgent env var
    L1_UNLOCK_REGISTRIES: gems,homebrew,npm,pypi,crates -> *
in seven monitored steps, mirroring the wave-2 shape:

  1. Read current L1_UNLOCK_REGISTRIES via PlistBuddy. Abort with
     STATUS: blocked unless the value is exactly
     'gems,homebrew,npm,pypi,crates' (the wave-2 end state).
  2. For each remaining software registry (14 of them: wordpress,
     chrome, ios, android, saas, ai_tool, nuget, steam, packagist, go,
     vscode, firefox, website, crypto) run
         scripts/dryrun_l1_kings_unlock.py --n-per-reg 50 --registries <reg>
     and parse the summary.json. Halt with STATUS: needs_approval if
     any registry has crashes or antipatterns > 0.
  3. PlistBuddy Set the env var to '*' (the full-sweep sentinel
     interpreted by agent_safety_pages as "unlock everywhere").
  4. launchctl unload + load (not just kickstart — the 2026-04-18
     incident confirmed kickstart alone does not propagate new
     EnvironmentVariables).
  5. Sleep 30s, then curl http://localhost:8000/v1/health and require
     status == 'ok'.
  6. Invoke scripts/purge_redis_canary.py WITHOUT SMEDJAN_CANARY_REGS
     set (full-sweep purge). The purge script's default cohort when the
     env var is absent is tracked by follow-up work — see NOTES in the
     task result if the in-tree script still defaults to
     'gems,homebrew' at execute time.
  7. Write an observation report skeleton to
     ~/smedjan/observations/L1-wave3-<UTC-ts>.md for canary_monitor_l1
...
PRE_UNLOCK  = "gems,homebrew,npm,pypi,crates"
POST_UNLOCK = "*"
NEW_REGS    = [
    "wordpress", "chrome", "ios", "android", "saas", "ai_tool",
    "nuget", "steam", "packagist", "go", "vscode", "firefox",
    "website", "crypto",
]
...
DRYRUN_OUT_BASE = Path(os.path.expanduser(
    os.environ.get("SMEDJAN_WAVE3_DRYRUN_DIR", "~/smedjan/discovery/canary-wave3")
))
...
log = logging.getLogger("smedjan.deploy_l1_wave3")
...
def verify_precondition() -> None:
    ...
    if current != PRE_UNLOCK:
        raise Blocked(
            ...
            f"(expected exactly {PRE_UNLOCK!r} — the wave-2 end state)"
        )
...
# ---------- step 6: redis canary purge (full-sweep) -----------------------

def purge_redis(execute: bool) -> dict[str, Any]:
    # Wave 3 is a full-sweep: deliberately DO NOT set SMEDJAN_CANARY_REGS.
    # The purge script is expected to treat an absent env var as "all
    # enriched registries" — if the in-tree script still defaults to
    # 'gems,homebrew' at execute time, that is a separate fix tracked
    # outside this task's whitelist.
    env = os.environ.copy()
    env.pop("SMEDJAN_CANARY_REGS", None)
    if not execute:
        env["SMEDJAN_DRY_RUN"] = "1"
    cmd = [sys.executable, str(PURGE_SCRIPT)]
    log.info("%s redis purge (full-sweep, no SMEDJAN_CANARY_REGS): %s",
             "EXEC" if execute else "DRY", shlex.join(cmd))
...
REPORT_SKELETON = """# L1 Wave 3 — Deploy Observation T+0 (full non-skip sweep)

**Generated:** {ts}
**Deploy T0:** {t0}
**Pre-unlock:** `{pre}`
**Post-unlock:** `{post}`
**New registries this wave ({n_new}):** {new_regs}
...
- Invoked WITHOUT `SMEDJAN_CANARY_REGS` (full-sweep).
...
## AI-bot crawls — wave-3 cohort (filled by canary_monitor_l1)

| Window | {ai_bot_header} |
|---|{ai_bot_sep}|
| 12h | {ai_bot_blanks} |
| 24h | {ai_bot_blanks} |
| 7d  | {ai_bot_blanks} |

Baseline (PRE, 7d): {baseline_line}

## Citations (human visits with AI-platform referrer) — wave-3 cohort
...
## 5xx observed — per-registry wave-3 cohort

| Registry | 12h total | 12h 5xx | 24h total | 24h 5xx |
|---|---:|---:|---:|---:|
{err_rows}
...
- T+0.5h — first gradient phase, halt if any registry crash>0 or site 5xx spike.
...
def _dryrun_rows(results: dict[str, dict[str, Any]] | None) -> str:
    if not results:
        return "\n".join(
            f"| {reg} | _ | _ | _ | _ | _ |" for reg in NEW_REGS
        )
    rows = []
    for reg in NEW_REGS:
        r = results.get(reg, {})
...
def _err_rows() -> str:
    return "\n".join(f"| {reg} | _ | _ | _ | _ |" for reg in NEW_REGS)
...
def write_observation(
    ...
    path = OBS_DIR / f"L1-wave3-{ts.strftime('%Y%m%dT%H%M%SZ')}.md"

    ai_bot_header = " | ".join(NEW_REGS + ["total"])
    ai_bot_sep = "|".join(["---:"] * (len(NEW_REGS) + 1))
    ai_bot_blanks = " | ".join(["_"] * (len(NEW_REGS) + 1))
    baseline_line = " / ".join(f"{reg}=_" for reg in NEW_REGS)
    ...
    body = REPORT_SKELETON.format(
        ...
        ai_bot_header=ai_bot_header,
        ai_bot_sep=ai_bot_sep,
        ai_bot_blanks=ai_bot_blanks,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="L1 Wave 3 (full non-skip sweep) deployment runner. "
    ...
    log.info("==== L1 Wave 3 deploy runner START (%s) ====", exec_mode)
    ...
    log.info("==== L1 Wave 3 deploy runner DONE (%s) ====", exec_mode)
    return 0
