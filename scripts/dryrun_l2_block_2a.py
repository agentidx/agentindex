#!/usr/bin/env python3
"""
dryrun_l2_block_2a.py — L2 Block 2a shadow-mode dry-run harness (T110).

Renders the Block 2a snippet for the top N slugs by `ai_demand_score` in
...
Usage (from repo root, so `smedjan.*` imports resolve):
    ...
    python3 scripts/dryrun_l2_block_2a.py            # default N=100
    python3 scripts/dryrun_l2_block_2a.py --limit 25 # smaller sample

The report is written to
`~/smedjan/audit-reports/l2-block-2a-dryrun-<UTC timestamp>.json` when
...
def _pick_top_slugs(limit: int) -> list[str]:
    """Top slugs by `ai_demand_score` that also have at least one
    external trust signal. Falls back to top-by-signal-count agents
    when the ai_demand × trust-signal overlap is shorter than `limit` —
    the dry-run is about block validation, not demand ranking, so we
    widen the pool rather than return a short sample.
    """
...
def main() -> int:
    ...
    if not slugs:
        print("no candidate slugs found (no ai_demand × external_trust_signals overlap)")
        return 1
    ...
    out_path = out_dir / f"l2-block-2a-dryrun-{stamp}.json"
    out_path.write_text(json.dumps(report, indent=2))

    print(f"task          : T110 L2 Block 2a dry-run")
    print(f"sample        : {len(slugs)} slugs (top by ai_demand_score, trust-signal-filtered)")
