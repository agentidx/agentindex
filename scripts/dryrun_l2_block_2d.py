#!/usr/bin/env python3
"""
dryrun_l2_block_2d.py — L2 Block 2d shadow-mode dry-run harness (T116).

Renders the Block 2d snippet for the top N slugs by `ai_demand_score`
(initial scope: top 500; default sample here is 100) in all three
...
Usage (from repo root, so `smedjan.*` imports resolve):
    ...
    python3 scripts/dryrun_l2_block_2d.py            # default N=100
    python3 scripts/dryrun_l2_block_2d.py --limit 25 # smaller sample

The report is written to
`~/smedjan/audit-reports/l2-block-2d-dryrun-<UTC timestamp>.json` when
...
from smedjan.renderers.block_2d import render_block_2d_html  # noqa: E402
...
def _pick_top_slugs(limit: int) -> list[str]:
    """Top slugs by `ai_demand_score`. The renderer itself is gated on
    the top-500 set; this harness mirrors the same ordering."""
...
def main() -> int:
    ...
    out_path = out_dir / f"l2-block-2d-dryrun-{stamp}.json"
    out_path.write_text(json.dumps(report, indent=2))

    print(f"task          : T116 L2 Block 2d dry-run")
