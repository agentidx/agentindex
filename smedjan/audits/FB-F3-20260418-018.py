"""Generate /compare/ coverage proposals for FB-F3-20260418-018.

Same shape as FB-F3-20260418-017/019: pulls highest-demand slugs from
...
Output: /Users/anstudio/smedjan/audits/FB-F3-20260418-018.md
"""
...
def main() -> int:
    ...
    # sorted_pair -> (combined_score, registry)
    ...
    lines = [
        "# FB-F3-20260418-018 /compare/ coverage proposals (top 50 pairs)",
        "",
        "Pairs drawn from the 50 highest-demand slugs within each registry "
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "Every pair returned `200` on a single curl — no retries were "
            "needed. Consistent with prior F3 audits: Nerq's `/compare/` "
            "route appears to serve a lightweight stub for pairs without a "
            "materialised analysis, so `200` alone is not conclusive evidence "
            "the page has real content. A body-inspection follow-up (e.g. "
            "detect `Not Yet Analyzed` markers) would split `skip` into "
            "`skip-real` vs `skip-stub`. No `404`s in this run means no "
            "`create` candidates surfaced.",
            "",
        ]
    )
    OUT.write_text("\n".join(lines))
