...
# --- 1. already-audited pairs across all sibling tasks ---
SIBLINGS = [1, 2, 3, 4, 5, 6, 8, 9, 10]
seen = set()
for i in SIBLINGS:
    p = Path.home() / f"smedjan/audits/FB-F3-20260419-{i:03d}.md"
    if not p.exists():
        continue
...
with sources.smedjan_db_cursor() as (_, cur):
    cur.execute("SELECT slug, score FROM smedjan.ai_demand_scores WHERE slug <> 'test' ORDER BY score DESC LIMIT 800")
...
for (a, b), (reg, score) in by_pair.items():
    # exclude if pair already audited in ANY registry in any sibling
    if any((r, a, b) in seen for r in list(by_reg.keys()) + [reg]):
        continue
...
for reg, a, b, code, _s in results:
    if code == "200":
        counts["200"] += 1
    elif code == "404":
        counts["404"] += 1
...
out_path = Path.home() / "smedjan/audits/FB-F3-20260419-007.md"
lines = [
    "# FB-F3-20260419-007 /compare/ coverage proposals (top 50 pairs)",
    "",
    "Fallback-generated. Top 50 demand-scored slugs per registry (smedjan.ai_demand_scores joined to Nerq `entity_lookup.source`) are cross-paired within each registry; each pair scored by combined demand; cross-registry duplicates collapse to the highest-scoring registry. Pairs already audited in sibling tasks FB-F3-20260419-001..006 and 008..010 are excluded so this run surfaces the next tier of proposals. Slug `test` is dropped (scraping artefact).",
    "",
...
lines.append("## Notes")
lines.append("")
lines.append(f"Excluded {len(seen)} pairs previously curled by sibling FB-F3-20260419 audits so this run covers the next-best proposals. Nerq's `/compare/` route may serve a lightweight stub page, so a `200` here means 'route responds' not 'analysis materialised' — body-inspection (detect `Not Yet Analyzed`) would split `skip` into `skip-real` vs `skip-stub` in a future refinement task.")
out_path.write_text("\n".join(lines) + "\n")
