"""FB-F1-20260419-021: random /safe/* antipattern spot-check.
...
Writes findings table to ~/smedjan/audits/FB-F1-20260419-021.md.
"""
...
def main() -> int:
    ...
    def record(name: str, slug: str) -> None:
    ...
    for slug in slugs:
        ...
        if status == 0:
            fetch_errors += 1
            record("fetch_error", slug)
            continue
        if status != 200 or not html:
            record("http_non_200", slug)
            continue
        f = scan(html)
    ...
    for name in order:
        ...
        samples_list = finding_samples[name]
        if samples_list:
            samples = ", ".join(f"`{s}`" for s in samples_list)
        else:
            samples = "\u2014"
