"""HTML-level quality audit of 50 existing /compare/ pages (FB-F3-20260419-028).
...
Outputs:
  ~/smedjan/audits/FB-F3-20260419-028.md
  ~/smedjan/audits/FB-F3-20260419-028.sampled_urls.jsonl
"""
...
MAX_CANDIDATE_PAIRS = 800        # cap before random shuffle
MAX_HEAD_PROBES = 300            # safety limit for status probes
EXCLUDE_SLUGS = {"test"}
USER_AGENT = f"smedjan-audit/1.0 (+{TASK_ID})"
BASE = "https://nerq.ai"
RANDOM_SEED = 20260419028
...
def load_prior_sampled_urls() -> set[str]:
    """URLs already audited in F3 JSONL files within the last 7 days."""
...
def fetch_top_slugs_per_registry() -> dict[str, list[str]]:
    """Top TOP_SLUGS_PER_REGISTRY demand slugs per Nerq registry."""
...
def enumerate_candidate_pairs(
    ...
    """All intra-registry pairs as URLs, de-duplicated and shuffled."""
    urls: set[str] = set()
...
def detect_king_sections(html: str, body_text: str) -> bool:
    if re.search(r"Detailed\s+Score\s+Analysis", html, re.IGNORECASE):
        return True
    # Otherwise require all 5 dim labels in a table-row context.
...
def detect_pplx_verdict(html: str) -> bool:
    # The task specifies <p class="pplx-verdict">; accept any element tag
    # carrying that class so we can distinguish true absence from a
    # surface-level element-type drift.
    if BeautifulSoup is None:
...
def fetch_enriched_at_map(all_slugs: list[str]) -> dict[str, datetime | None]:
    """max(enriched_at) per slug across software_registry rows."""
    if not all_slugs:
        return {}
    with sources.nerq_readonly_cursor() as (_, cur):
        cur.execute(
