"""HTML-level quality audit of 50 existing /compare/ pages (FB-F3-20260423-016).
...
Outputs:
  ~/smedjan/audits/FB-F3-20260423-016.md
  ~/smedjan/audits/FB-F3-20260423-016.sampled_urls.jsonl
"""
...
from smedjan import config as smedjan_config  # noqa: E402

# Local Nerq Postgres is reachable on port 6432 (port 5432 not bound on this
# host right now); override the DSN from config.toml so the sources module
# still routes through its normal helpers. Uses local anstudio/trust auth —
# peer trust is fine for a read-only SELECT, and smedjan_readonly isn't
# reachable on this port.
smedjan_config.NERQ_RO_DSN = "postgresql://anstudio@localhost:6432/agentindex"
