...
_TEMPLATES: dict[str, dict[str, str]] = {
    ...
    "F3": {
        ...
        "description": (
            ...
            "instead grades the pages that already exist.\n\n"
            "Sampling (via Nerq read-only source; sources.get_nerq_readonly()):\n"
            "  1. Select 50 random EXISTING /compare/<a>-vs-<b> URLs. Pick "
            "     from public.comparisons OR by sampling pairs that returned "
            "     HTTP 200 on a quick pre-check. If comparisons table is "
            "     unavailable, fall back to: pair top-100 demand slugs per "
            "     registry, curl /compare/<a>-vs-<b> HEAD, keep the first "
            "     50 that return 200.\n"
            "  2. Avoid duplicates within the last 7 days — read prior "
            ...
            "    per sampled URL, plus a leading summary section with counts "
            "    (pages missing king_sections, missing sacred bytes, word "
            "    count distribution [p25/p50/p75], data_density p50, "
            "    staleness p50 and count(staleness > 30d)). Include an "
            "    'Escalation list' section listing the URLs that fail any of: "
            "    missing king_sections, missing pplx-verdict, missing "
            "    ai-summary, missing FAQ JSON-LD, word_count < 200, "
            "    data_density < 2, staleness > 60d.\n"
            "  - ~/smedjan/audits/{task_id}.sampled_urls.jsonl: one JSON "
            "    object per line with {url, has_king_sections, "
            "    has_pplx_verdict, has_ai_summary, has_faq_jsonld, "
            "    word_count, number_tokens, data_density, last_enriched_at, "
            "    staleness_days}.\n\n"
        ...
        "acceptance_criteria": (
            ...
            "exactly 50 lines. The markdown has a summary section + "
            "findings-table + escalation-list section. No sampled URL "
            "appears in the last 7 days of prior F3 audit JSONL files. "
            "Slug 'test' appears in neither file. EVIDENCE reports "
            "pages_audited=50, escalation_count=<int>, p50_data_density, "
            "p50_staleness_days."
        ),
    },
}
