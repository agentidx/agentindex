...
_TEMPLATES: dict[str, dict[str, str]] = {
    ...
    "F3": {
        # F3-v3 (post-L1b canary), 2026-04-23: restricts the sample to
        # pairs where BOTH slugs sit inside the L1b canary cohort
        # (registries listed in the service env var
        # L1B_COMPARE_UNLOCK_REGISTRIES, default 'npm,pypi'). Prior v2
        # sampled across ALL registries, so <1% of the 50-URL pool landed
        # inside the canary scope and the missing_king / missing_pplx
        # summary was dominated by out-of-scope noise rather than real
        # canary coverage. The env-var is read at runtime from
        # ~/Library/LaunchAgents/com.nerq.api.plist so a future canary
        # expansion lands without a template edit — do NOT hardcode the
        # registry list in the audit script.
        "title": "F3-v3 compare_l1b_verification: HTML audit of 50 /compare/ pages inside L1b canary cohort",
        "description": (
            "Fallback-generated post-L1b verification audit of /compare/ "
            "pages. Samples 50 random existing /compare/<a>-vs-<b> URLs "
            "where BOTH slugs sit inside the L1b canary cohort so the "
            "missing_king + missing_pplx summary reflects real canary "
            "coverage instead of out-of-scope noise.\n\n"
            "Canary cohort resolution (runtime):\n"
            "  Read the running value of the service env var "
            "  L1B_COMPARE_UNLOCK_REGISTRIES from "
            "  ~/Library/LaunchAgents/com.nerq.api.plist (look for a "
            "  <key>L1B_COMPARE_UNLOCK_REGISTRIES</key> followed by a "
            "  <string>...</string> inside EnvironmentVariables). Parse as "
            "  comma-separated, lower-cased, whitespace-trimmed. Fallback "
            "  to 'npm,pypi' ONLY if the plist read fails — never "
            "  hardcode the pair in any branching logic, the plist is the "
            "  source of truth so a future expansion Just Works.\n\n"
            "Sampling:\n"
            "  1. Against smedjan DB (sources.smedjan_db_cursor()), pull "
            "     the top-3000 slugs from smedjan.ai_demand_scores ordered "
            "     by score DESC (excluding slug 'test').\n"
            "  2. Against Nerq RO (sources.nerq_readonly_cursor()), join "
            "     those slugs to public.entity_lookup to get (slug, "
            "     source). Restrict to rows where source IS IN the canary "
            "     cohort resolved above — this is the filter that did NOT "
            "     exist in v2.\n"
            "  3. Group the surviving slugs by source; take the top-100 "
            "     by demand score per registry (same cap as v2, but now "
            "     applied only inside the canary).\n"
            "  4. Enumerate candidate /compare/<a>-vs-<b> URLs by taking "
            "     combinations(slugs, 2) within each canary registry AND "
            "     across canary registries — both slugs must be in the "
            "     canary set, registries can differ. Exclude pairs whose "
            "     URL appears in any ~/smedjan/audits/FB-F3-*"
            ".sampled_urls.jsonl from the last 7 days; exclude slug "
            "     'test'.\n"
            "  5. Shuffle deterministically (seed = last 9 digits of "
            "     {task_id}), HEAD /compare/<a>-vs-<b> on https://nerq.ai "
            "     with User-Agent 'smedjan-audit/1.0', keep the first 50 "
            "     that return 200. If fewer than 50 qualify (narrow "
            "     canary), carry on with what you have and record the "
            "     shortfall in EVIDENCE.shortfall.\n\n"
            "For each sampled URL, curl the full HTML from https://nerq.ai "
            "and extract:\n"
            "  - has_king_sections: bool — is the 5-dim Detailed-Score-"
            "    Analysis table present? (heading 'Detailed Score "
            "    Analysis' OR a table with rows Security/Maintenance/"
            "    Popularity/Quality/Community).\n"
            "  - has_pplx_verdict:  bool — class=\"pplx-verdict\" "
            "    (attribute-order agnostic).\n"
            "  - has_ai_summary:    bool — class=\"ai-summary\".\n"
            "  - has_faq_jsonld:    bool — a <script type=\"application/"
            "    ld+json\"> block whose json.loads() yields "
            "    '@type':'FAQPage'.\n"
            "  - word_count:        int  — text-only words in <body>.\n"
            "  - number_tokens:     int  — integer + decimal tokens in "
            "    the body text.\n"
            "  - data_density:      float — number_tokens per 1000 words.\n"
            "  - last_enriched_at:  iso | null — max(enriched_at) across "
            "    slug_a + slug_b from public.software_registry.\n"
            "  - staleness_days:    int | null — days since "
            "    last_enriched_at.\n"
            "  - registry_a, registry_b: str — sources from "
            "    entity_lookup for each slug. BOTH must be in the "
            "    resolved canary-registry set; if any sampled row's "
            "    registry_a or registry_b falls outside the set the "
            "    sampler has leaked and the run must fail with "
            "    STATUS: blocked (note: 'sampler leaked "
            "    out-of-cohort rows').\n\n"
            "Write two files:\n"
            "  - ~/smedjan/audits/{task_id}.md: leading summary section "
            "    with canary_registries=<resolved set>, pages_audited, "
            "    missing_king_sections (count/N), missing_pplx_verdict "
            "    (count/N), missing_ai_summary, missing_faq_jsonld, "
            "    word_count p25/p50/p75, data_density p50, "
            "    staleness_days p50 and count(staleness > 30d). Findings "
            "    table with one row per sampled URL (url, king, pplx, "
            "    ai_sum, faq, words, nums, density, last_enriched, "
            "    stale_d, registry_a, registry_b). Escalation-list "
            "    section with URLs that miss king OR pplx OR any sacred "
            "    byte OR have word_count < 200 OR data_density < 2 OR "
            "    staleness > 60d.\n"
            "  - ~/smedjan/audits/{task_id}.sampled_urls.jsonl: one JSON "
            "    object per line with every extracted field above PLUS "
            "    the resolved canary_registries list so downstream "
            "    readers can see what cohort the run targeted.\n\n"
            "STATUS: done if both files exist, N rows written (N<=50, "
            "short counts permitted), EVIDENCE reports canary_registries "
            "+ pages_audited + missing_king_count + missing_pplx_count, "
            "and every row's (registry_a, registry_b) are both inside "
            "the canary set. STATUS: needs_approval if > 20 of 50 pages "
            "are on the escalation list — a systemic regression worth "
            "human review under the post-L1b contract. STATUS: blocked "
            "if any sampled row's registry falls outside the canary "
            "set (sampler bug)."
        ),
        "acceptance_criteria": (
            "Two files exist: ~/smedjan/audits/{task_id}.md and "
            "~/smedjan/audits/{task_id}.sampled_urls.jsonl. JSONL has "
            "up to 50 rows (shortfall allowed for narrow canary; "
            "recorded in EVIDENCE.shortfall). Every row carries "
            "registry_a and registry_b, both of which appear in the "
            "canary-registry set resolved from "
            "L1B_COMPARE_UNLOCK_REGISTRIES at runtime (fallback "
            "'npm,pypi'). Summary section reports missing_king + "
            "missing_pplx counts reflecting real canary coverage. "
            "EVIDENCE reports canary_registries, pages_audited, "
            "missing_king_count, missing_pplx_count. Slug 'test' "
